...
async function main() {
  ...
  const r = await s.process.executeCommand('cd /workspace && timeout 10 bun run dev --host 0.0.0.0 2>&1 || true', '/workspace');
  ...
  // Check vite
  const vite = await s.process.executeCommand('ls node_modules/.bin/vite 2>/dev/null && node_modules/.bin/vite --version 2>/dev/null || echo "no vite"', '/workspace');
  console.log('\nvite:', vite.result);
}
