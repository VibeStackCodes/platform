...
async function main() {
  ...
  const logs = await s.process.executeCommand('cat /tmp/checker-errors.log 2>/dev/null || echo "no log file"', '/workspace');
  console.log('\n=== checker-errors.log ===');
  console.log(logs.result);

  // Try curl localhost:3000
  const curl = await s.process.executeCommand('curl -s -o /dev/null -w "%{http_code}" http://localhost:3000 2>/dev/null || echo "FAILED"', '/workspace');
  console.log('\n=== curl localhost:3000 ===');
  console.log(curl.result);

  // Check package.json scripts
  const pkg = await s.process.executeCommand('cat /workspace/package.json 2>/dev/null || echo "no package.json"', '/workspace');
  console.log('\n=== package.json ===');
