...
export function contractToSeedSQL(
...
function inferTextValue(name: string, n: number): string {
  ...
  if (name.includes('description') || name.includes('content') || name.includes('body') || name.includes('note'))
    return `This is sample ${name.replace(/_/g, ' ')} for row ${n}.`;
  if (name.includes('status'))
    return ['active', 'pending', 'completed', 'inactive', 'draft'][n % 5];
  if (name.includes('type') || name.includes('category') || name.includes('role'))
    return `type_${n}`;
  if (name.includes('address'))
    return `${n * 100} Main Street`;
  if (name.includes('city'))
    return ['New York', 'San Francisco', 'Chicago', 'Austin', 'Seattle'][n % 5];
  if (name.includes('color'))
    return ['red', 'blue', 'green', 'purple', 'orange'][n % 5];
