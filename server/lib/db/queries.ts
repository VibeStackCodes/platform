...
export async function updateProject(projectId: string, fields: Partial<typeof projects.$inferInsert>) {
...
export async function updateProfileByStripeId(
  ...
  fields: Partial<Pick<typeof profiles.$inferInsert, 'plan' | 'creditsMonthly' | 'creditsRemaining' | 'creditsResetAt'>>,
) {
  return db.update(profiles)
    .set(fields)
    .where(eq(profiles.stripeCustomerId, stripeCustomerId))
...
export async function getStripeCustomerId(userId: string) {
  return db.select({ stripeCustomerId: profiles.stripeCustomerId })
    .from(profiles)
    .where(eq(profiles.id, userId))
