...
function routeFilePathFromPage(page: PageDef): string | null {
  if (page.type === 'public-list' && page.entity) {
    return `src/routes/${snakeToKebab(pluralize(page.entity))}/index.tsx`
  }
  if (page.type === 'public-detail' && page.entity) {
    return `src/routes/${snakeToKebab(pluralize(page.entity))}/$id.tsx`
  }
  if (page.type === 'crud-list' && page.entity) {
    return `src/routes/_authenticated/${snakeToKebab(pluralize(page.entity))}/index.tsx`
  }
  if (page.type === 'crud-detail' && page.entity) {
    return `src/routes/_authenticated/${snakeToKebab(pluralize(page.entity))}/$id.tsx`
  }
  if (page.path === '/') return 'src/routes/index.tsx'
  if (page.type === 'static') return `src/routes/${page.path.replace(/^\//, '')}.tsx`
  return null
}

function routeImportPathFromSource(filePath: string): string {
  return `./${filePath.replace(/^src\//, '').replace(/\.tsx$/, '')}`
}

function routeTreeFromAssemblyPages(
  assembly: AssemblyResult,
  availableRouteFiles: Set<string>,
): string {
  const lines: string[] = [
    '/* eslint-disable */',
    '// @ts-nocheck',
  ...
  const filteredPages = assembly.pages.filter((page) => {
    const filePath = routeFilePathFromPage(page)
    return filePath ? availableRouteFiles.has(filePath) : false
  })

  const routeRefs: Array<{ key: string; constName: string; parentConst: string; path: string }> = []
  ...
  for (const page of filteredPages) {
    ...
    const importPath = routeImportPathFromSource(filePath)
    if (!addedImports.has(importPath)) {
      const importName = `RouteImport${++indexCounter}`
      lines.push(`import { Route as ${importName} } from '${importPath}'`)
      addedImports.add(importPath)
      routeRefs.push({
        key: importPath,
        constName: `${importName}Node`,
        parentConst: page.path.startsWith('/_authenticated') ? 'AuthenticatedRoute' : 'rootRoute',
        path: page.path,
      })
    } else {
      const existing = routeRefs.find((ref) => ref.key === importPath)
      if (existing) continue
    }
  }

  const hasAuthParent = filteredPages.some((page) => page.path.startsWith('/_authenticated'))
  let hasAuthenticatedConst = false
  if (hasAuthParent) {
    const authFile = 'src/routes/_authenticated/route.tsx'
    if (availableRouteFiles.has(authFile) && !addedImports.has(routeImportPathFromSource(authFile))) {
      lines.push(`import { Route as AuthenticatedImport } from '${routeImportPathFromSource(authFile)}'`)
      routeRefs.push({
        key: routeImportPathFromSource(authFile),
        constName: 'AuthenticatedRoute',
        parentConst: 'rootRoute',
        path: '/_authenticated',
      })
      addedImports.add(routeImportPathFromSource(authFile))
      hasAuthenticatedConst = true
    }
  }

  if (!hasAuthenticatedConst) {
    for (const ref of routeRefs) {
      if (ref.parentConst === 'AuthenticatedRoute') ref.parentConst = 'rootRoute'
    }
  }

  routeRefs.sort((a, b) => {
    if (a.constName === 'AuthenticatedRoute') return -1
    if (b.constName === 'AuthenticatedRoute') return 1
    return 0
  })
  ...
  for (const ref of routeRefs) {
    const importName = ref.constName === 'AuthenticatedRoute'
      ? 'AuthenticatedImport'
      : ref.constName.replace('Node', '')
    const update = ref.constName === 'AuthenticatedRoute'
      ? `{ id: '/_authenticated', getParentRoute: () => rootRoute }`
      : `{ path: '${ref.path}', getParentRoute: () => ${ref.parentConst} }`
    lines.push(`const ${ref.constName} = ${importName}.update(${update} as any)`)
  }

  const navOrder = new Map(assembly.navEntries.map((entry) => [entry.path, entry.order ?? 50]))

  const privateChildren = routeRefs.filter((ref) =>
    ref.path.startsWith('/_authenticated') && ref.constName !== 'AuthenticatedRoute')
  const publicChildren = routeRefs
    .filter((ref) =>
    !ref.path.startsWith('/_authenticated') && ref.constName !== 'AuthenticatedRoute')
    .sort((a, b) => {
      const orderA = navOrder.get(a.path) ?? 50
      const orderB = navOrder.get(b.path) ?? 50
      return orderA - orderB || a.path.localeCompare(b.path)
    })

  lines.push('')
  lines.push('export const routeTree = rootRoute.addChildren([')
  for (const child of publicChildren) lines.push(`  ${child.constName},`)
  if (routeRefs.some((ref) => ref.constName === 'AuthenticatedRoute')) {
    lines.push('  AuthenticatedRoute.addChildren([')
    for (const child of privateChildren) lines.push(`    ${child.constName},`)
    lines.push('  ]),')
  }
  lines.push('])')
  lines.push('')
...
function generateRouteTree(contract: SchemaContract, features: InferredFeatures, tokens: ThemeTokens, assembly?: AssemblyResult | null, availableRouteFiles?: Set<string>): string {
  if (assembly?.pages && assembly.pages.length > 0 && availableRouteFiles) {
    return routeTreeFromAssemblyPages(assembly, availableRouteFiles)
  }
...
`,
  ...
  fileTree.push({
    ...
    content: generateRouteTree(input.contract, features, tokens, input.assembly, availableRouteFiles),
    layer: 5,
    isLLMSlot: false,
  })
  fileTree.push({
