  ...
  test('full flow: sign up → chat → plan → generate', async ({ page }) => {
    ...
    await test.step('Answer questions until plan appears', async () => {
      ...
      const screenshotInterval = setInterval(async () => {
        const ts = Date.now()
        await page
          .screenshot({ path: `test-results/waiting-${ts}.png`, fullPage: true })
          .catch(() => {})
    ...
    await test.step('Approve and generate', async () => {
      await page
        .getByRole('button', { name: /Approve & Generate/i })
        .last()
        .click()

      // ConfirmationAccepted shows approval state
      await expect(page.getByText(/Plan approved|generating/i).first()).toBeVisible({
        timeout: 10_000,
      })
    ...
    await test.step('Watch generation queue', async () => {
      ...
      await expect(page.getByText(/\(\d+ lines\)/).first()).toBeVisible({ timeout: 300_000 })
