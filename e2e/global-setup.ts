...
async function cleanSupabase() {
  ...
  if (!token || !orgId) {
    console.log('[global-setup] Skipping Supabase cleanup — missing SUPABASE_ACCESS_TOKEN or SUPABASE_E2E_ORG_ID');
    return;
  }
  ...
  const projects: Array<{ id: string; name: string; organization_id: string; status: string }> = await res.json();
  ...
  console.log(`[global-setup] Found ${toDelete.length} Supabase project(s) to delete (of ${projects.length} total)`);

  for (const project of toDelete) {
    console.log(`[global-setup] Deleting Supabase project: ${project.name} (${project.id}) [org: ${project.organization_id}]`);
...
async function cleanVercel() {
  ...
  try {
    ...
    console.log(`[global-setup] Found ${toDelete.length} Vercel project(s) to delete (of ${projects.length} total)`);
