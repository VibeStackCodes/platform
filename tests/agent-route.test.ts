...
import { createClient, getUser } from '@server/middleware/auth'
...
describe.skip('POST /api/agent', () => {
  ...
  it('bridges agent-execution-end to agent_complete', async () => {
    ...
    async function* chunks() {
      yield {
        ...
        payload: { agentId: 'analyst', agentName: 'Analyst', tokensUsed: 150, durationMs: 2000 },
      }
    }
    mockNetwork.mockResolvedValue(createMockExecution(chunks()))
  ...
  it('bridges tool-execution-end write-file to file_complete', async () => {
    ...
    async function* chunks() {
      yield {
        ...
        payload: {
          toolName: 'write-file',
          result: { path: '/workspace/src/App.tsx', bytesWritten: 42 },
  ...
  it('bridges non-write tool-execution-end to agent_artifact', async () => {
    ...
    async function* chunks() {
      yield {
        type: 'tool-execution-end',
        payload: { toolName: 'run-build', agentId: 'qa-engineer' },
      }
    }
    mockNetwork.mockResolvedValue(createMockExecution(chunks()))
  ...
  it('bridges agent-execution-event-text-delta to agent_progress', async () => {
    ...
    async function* chunks() {
      yield {
        ...
        payload: { agentId: 'analyst', textDelta: 'Analyzing requirements...' },
      }
    }
    mockNetwork.mockResolvedValue(createMockExecution(chunks()))
  ...
  it('bridges routing-agent-start to stage_update planning', async () => {
    ...
    const planningEvent = events.find(
      (e: any) => e.type === 'stage_update' && e.stage === 'planning',
    )
  ...
  it('bridges routing-agent-end to checkpoint with delegation target', async () => {
    ...
    const checkpoint = events.find(
      (e: any) => e.type === 'checkpoint' && e.label?.includes('analyst'),
    )
    expect(checkpoint).toEqual({
      type: 'checkpoint',
  ...
  it('bridges network-execution-event-finish to pipeline complete checkpoint', async () => {
    ...
    const checkpoint = events.find(
      (e: any) => e.type === 'checkpoint' && e.label === 'Pipeline complete',
    )
  ...
  it('bridges workflow-execution-suspended to plan_ready', async () => {
    ...
    async function* chunks() {
      yield {
        ...
        payload: { suspendPayload: { appName: 'Todo App', features: [] } },
      }
    }
    mockNetwork.mockResolvedValue(createMockExecution(chunks()))
  ...
  it('handles missing payload fields gracefully', async () => {
    ...
    const agentStart = events.find((e: any) => e.type === 'agent_start') as
    ...
    const agentComplete = events.find((e: any) => e.type === 'agent_complete') as
      | Record<string, unknown>
      | undefined
    ...
    const artifact = events.find((e: any) => e.type === 'agent_artifact') as
      | Record<string, unknown>
      | undefined
  ...
  it('handles multiple chunks sequentially', async () => {
    ...
    async function* chunks() {
      ...
      yield {
        type: 'agent-execution-event-text-delta',
        payload: { agentId: 'analyst', textDelta: 'Step 1' },
      }
      yield {
        type: 'agent-execution-event-text-delta',
        payload: { agentId: 'analyst', textDelta: 'Step 2' },
      }
      yield {
        type: 'agent-execution-end',
        payload: { agentId: 'analyst', tokensUsed: 100, durationMs: 1000 },
      }
    }
    mockNetwork.mockResolvedValue(createMockExecution(chunks()))
  ...
  it('deducts credits using accurate usage from network execution', async () => {
    ...
    async function* chunks() {
      yield {
        type: 'agent-execution-end',
        payload: { agentId: 'analyst', usage: { totalTokens: 5000 }, durationMs: 1000 },
      }
      yield {
        type: 'agent-execution-end',
        payload: { agentId: 'frontend', usage: { totalTokens: 3000 }, durationMs: 2000 },
      }
    }
    mockNetwork.mockResolvedValue(
      createMockExecution(chunks(), {
        inputTokens: 5600,
        outputTokens: 2400,
        totalTokens: 8000,
      }),
