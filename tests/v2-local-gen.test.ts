/**
 * V2 Local Generation Test — Real Services, No UI
 *
 * Calls the V2 orchestrator agent directly with real Daytona sandbox,
 * real LLM, and real Relace. Bypasses HTTP route (no auth/credits/DB).
 *
 * Run manually:
 *   bun run test -- tests/v2-local-gen.test.ts --timeout 300000
 */
...
import { createV2Orchestrator } from '@server/lib/agents/v2-orchestrator'
...
describeReal(
  'V2 Local Generation (real services)',
  () => {
    it(
