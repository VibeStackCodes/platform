...
describe('searchWebTool', () => {
  it('returns search results placeholder', async () => {
    const { searchWebTool } = await import('@server/lib/agents/tools')
    const result = await searchWebTool.execute!({ query: 'Procore dashboard UI' }, {} as any)
    expect(result.results).toContain('Procore dashboard UI')
  })
})
