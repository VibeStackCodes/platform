...
describe('Sandbox URLs Routes', () => {
  ...
  beforeEach(() => {
    ...
    // Default: ownership check passes, project has a sandboxId
  ...
  describe('GET /api/projects/:id/sandbox-urls', () => {
    it('returns proxy preview URL and proxy code server URL for project with sandbox', async () => {
      ...
      vi.mocked(getSandbox).mockResolvedValue(mockSandbox as any)
    ...
    it('returns null URLs when project has no sandboxId', async () => {
      vi.mocked(getProject).mockResolvedValue({
        id: 'proj-1',
        userId: 'user-123',
        sandboxId: null,
    ...
    it('returns null URLs when getSandbox fails (sandbox deleted)', async () => {
    ...
    it('handles Promise.all rejection gracefully', async () => {
      ...
      vi.mocked(getProject).mockResolvedValue({
        id: 'proj-8',
        userId: 'user-123',
        sandboxId: 'sandbox-parallel-fail',
      } as any)
